...
// 4. Find the greatest increase in profits (date and difference in the amounts) over the entire period.
var greatestIncrease = finances.reduce((acc, entry, index, array) => {
  if (index === 0) return acc;

  let difference = entry[1] - array[index - 1][1];
  if (difference > acc.amount) {
    return { date: entry[0], amount: difference };
  }
  return acc;
}, { date: '', amount: -Infinity });

// 5. Find the greatest decrease in losses (date and difference in the amounts) over the entire period.
var greatestDecrease = finances.reduce((acc, entry, index, array) => {
  if (index === 0) return acc;
